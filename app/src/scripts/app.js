...
htmlElements.toggleAppBtns.forEach(toggleAppBtn => 
  toggleAppBtn.addEventListener('click', () => {
      if(!isTimerMode) {
        ...
        htmlElements.progressBarChildren.forEach(a => a.style.animation = "progressBar .7s ease-in-out forwards")
        setTimeout(() => {htmlElements.progressBarChildren.forEach(a => a.style.animation = null)}, 500);
        isTimerMode = true
      } else {
